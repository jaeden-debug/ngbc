...
- Hunt Brief live persistence requires a provisioned Supabase project with the two migrations applied. `HUNT_SHARE_RATE_LIMIT_SECRET` is configured, but `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are absent. Missing storage fails closed with `503`; no share URL is claimed until the snapshot is durably stored.
- No Google Maps browser key is configured, so the Hunt map currently runs its basemap-free boundary view. Official zone geometry, zone resolution, evaluation and every result are unaffected; satellite/terrain context and Google Places suggestions are not available until a key is supplied.
...
- Supabase project: no North Ground project exists in the connected Supabase account, and the connector lists only unrelated projects. No other project was touched. The owner must create the North Ground project, then apply `supabase/migrations/20260920000100_hunt_foundation.sql` and `supabase/migrations/20260920000200_certified_ontario_wmu57_2026.sql`, and add `SUPABASE_URL` plus `SUPABASE_SERVICE_ROLE_KEY` to Vercel Production and Preview. The service-role key is server-only. The schema is certified locally by `npm run certify:supabase` (33 assertions against a throwaway PostGIS database with the Supabase role model).
- Google Maps Platform keys: the owner must create two separately restricted keys and add `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` (browser key, HTTP-referrer restricted to the production and preview hosts, limited to the Maps JavaScript API and Places API (New)) and `GOOGLE_MAPS_SERVER_API_KEY` (server key, IP-restricted or unrestricted-but-server-only, limited to Places API (New), Geocoding API and Weather API). Set `GEOCODING_PROVIDER=google` and, if Google Weather is wanted, `WEATHER_PROVIDER=google` with `WEATHER_FALLBACK_PROVIDER=open-meteo`. Required APIs and restrictions are listed in `.env.example`. Hunt runs without them in a reduced but honest state.
...
- Database: Supabase (PostgreSQL + PostGIS in the `extensions` schema) is the selected application database for Hunt Brief snapshots, share rate limiting and normalized zone geometry. Migrations and local certification exist; no remote project is provisioned, so the live path is fail-closed.
...
- Map: Google Maps JavaScript API when a browser key is configured, with North Ground's own regulatory overlays on top; otherwise a basemap-free boundary view rendering the same official geometry. Neither is a legal survey.
- Geocoding / place search: Google Places API (New) and Geocoding API when a server key is configured; Nominatim otherwise. Provider choice is visible in the attribution and never changes regulatory truth.
- Weather: Google Weather API when configured, Open-Meteo otherwise. Environmental context only, current day through the provider's horizon; no climatology substitution.
- Analytics:
