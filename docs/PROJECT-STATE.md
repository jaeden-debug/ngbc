...
Last updated: 2026-09-21 (National integration: the migration ledger and the repository reconciled at 18 = 18 with the schema replayed identical to production; Ontario 7/7, Alberta 13/13 and Manitoba 24/24 production regressions agree with the law; the evaluation API accepts exactly the species a served jurisdiction certifies; production's Google server key is being refused, so weather and place search are running on their fallbacks (owner action); the follow-up question answers the arrow keys. Earlier: Ready to Hunt: licences, hunter orange, legal methods and ammunition, verified fees and an official licence-issuer search for every permitted Ontario hunt, with the vendor-search location kept apart from the hunt location; Hunt Brief v3; Ontario turkey now admits a muzzle-loading shotgun per s. 79. Earlier: Map follow-ups: zone cards account for restricted areas inside a zone; Ontario wide views no longer lose units; Québec map geometry reads stored drawings. Earlier: Hunt map became an exploration surface: official zone labels, zone cards from the canonical engine, device location kept apart from the hunt location, drop-pin with confirmation, species filter, Manitoba special-area layers. Earlier: Manitoba served and production-certified: 62 GHAs parity 201/0, grouse and white-tailed deer from M.R. 165/91, 24/24 live cases agree with the law, rules persisted and read back identical, change drill 8/8; the map draws every served jurisdiction. Earlier the same day: Alberta served; species coverage per jurisdiction; Supabase outage 02:50–03:24 UTC resolved)
...
- Owner decision: the Québec resolver swap (serving `resolve_management_zone_v2`, which also fixes the boundary distance) awaits approval in the Québec session.
