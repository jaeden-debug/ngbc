...
Last updated: 2026-09-21 (Québec certified but not served: 59 zones in PostGIS, parity 306/0, 186 rules for 10 species persisted by year and read back identical, the ministry's closed territories checked at the point, zone layer and pages watched daily, 20 production cases written from the law and passing in process; serving waits on the owner's approval of the resolver swap, promotion and deploy. Map-intelligence foundation: blueprint now establishes EXPLORE / FIND GAME / CHECK HUNT, SPECIE HEAT MAP, Crown/Public Land, opportunity/land/access separation and progressive layer architecture. Canonical evidence/land/access/PostGIS contracts and a bounded opportunity API landed locally; official Ontario white-tailed deer harvest evidence is integrated as PARTIAL DATA. No map UI or new spatial dataset is deployed. Earlier: national integration reconciled the migration ledger and production schema at 18 = 18; Ontario, Alberta and Manitoba production regressions agree with the law; Ready to Hunt and Hunt Brief v3 landed; map exploration, Manitoba and Alberta remain production-certified.)
...
- **Ontario turkey in the Supabase mirror predates the s. 79 fix** (2026-09-21). The major-game bundle now permits a muzzle-loading shotgun (10–20 gauge, shot 4–7) in the spring and fall "shotgun or bow" turkey seasons, which the old parse had closed. Hunt evaluates from the bundle, so answers are already right; the mirror needs `node scripts/publish-regulations.mjs` for the major-game bundle (a production write, not yet run).
