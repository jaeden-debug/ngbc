...
Last updated: 2026-09-21 (Alberta geometry published and first rules built; species coverage per jurisdiction; Supabase outage open)
...
- **Supabase outage, 2026-09-21 from 02:50 UTC (open at the time of writing).** REST answers 522 after ~20 s and the management API cannot connect; the project still reports `ACTIVE_HEALTHY`. The logs show a 271-second checkpoint, then at 02:49:09 a `POST /zone_ingest_features` failing on statement timeout, then no logging at all after 02:50:46. The preceding half hour carried three large geometry ingests into a small instance — Manitoba (~624k vertices), Alberta (two staging runs of ~692k vertices and a publish) and, by the timing, a further upload — which is consistent with exhausting the instance's disk-I/O budget. It may need a restart or more compute from the Supabase dashboard; that is an owner decision. Production still answers correctly through the official-GIS fallback, but each evaluation waited ~20 s for the Supabase call to fail; a 2.5 s abort on that lookup is being added with the regulatory registry. Lesson for every ingest: stage one jurisdiction at a time, and batch very large MultiPolygons (Québec's 19SE is 8,091 polygons) small enough to stay inside the statement timeout.
...
**Alberta — geometry published, first rules built, not yet served (2026-09-21).**
...
- All 189 are published in PostGIS through the generalised promotion (`734bfef`, `16336a3`), with the jurisdiction and source registered by migration. **Parity is NOT certified**: the first live run reached ~550 of 592 points before a network failure and wrote no record, and the re-run is blocked by the Supabase outage. Until it certifies, Alberta's layer is not served and Hunt answers every Alberta point as not yet covered.
...
- Coverage: ruffed grouse 179 of 189 WMUs, spruce grouse 177, sharp-tailed grouse 92, white-tailed deer 177; the rest answer UNKNOWN. Not encoded: mule deer, moose, elk, sheep, goat, pronghorn, black bear, cougar, other game birds, migratory birds.
...
1. When Supabase answers again, run `node scripts/certify-spatial-parity.mjs --jurisdiction ca-ab` (about 600 points, a few minutes now) and commit `fixtures/hunt/ca-ab-spatial-parity.json` with a replay test like `src/lib/hunt/manitoba-spatial-parity.test.ts`. Do not serve Alberta on anything less than zero disagreements.
2. In one commit: add Alberta's `ZONE_LAYERS` entry (`serving: true`, the Alberta FeatureServer as endpoint, `WMUNIT_CODE` as name field), add `{ jurisdictionId: "jurisdiction:ca-ab", evaluate: evaluateAlberta, coverage: albertaCoverageReport }` to the regulatory registry, and list `regulatory_bundle:ca-ab-2026` in `src/lib/hunt/canada/registry.ts`. They go together so coverage never claims a jurisdiction Hunt cannot answer.
3. The map labels features by the raw name field, and Alberta's service stores `00102` for WMU 102: normalise it (`normalizeAlbertaWmuCode`) before the label and before `zoneCoverage`. The viewport route draws only the first layer that answers per view; a view spanning Alberta and another province shows one of them.
4. Next species, from the same tables and builder: mule deer (adds antler-point classes), moose (general and ■ by unit), then elk once the online/PDF divergence on late seasons is settled with Alberta.
5. Everything the builder needs is in `scripts/alberta-source.mjs`; re-run `scripts/crosscheck-alberta-guide.py` whenever the PDF hash changes, because the builder refuses a cross-check made against a different PDF.
