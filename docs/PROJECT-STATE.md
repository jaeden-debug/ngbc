...
- Migration `20260921113816_canonical_species_primary_media.sql` is in the live ledger and the private WebP-only, 15 MiB-limited bucket is provisioned. `20260921093347_map_intelligence_foundation.sql` remains intentionally unapplied and was not crossed while reconciling the later remote migration history. The production app has the publishable key and media session secret. Permanent administrator activation is **BLOCKED** only on the owner's administrator email: no real Auth user currently exists and `SPECIES_MEDIA_ADMIN_USER_IDS` was removed after certification, so the surface fails closed. Production remains 0/60 after all temporary certification assets and users were deleted.
