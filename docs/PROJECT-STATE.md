...
Last updated: 2026-09-21 (Canonical species PRIMARY media application code is live in Vercel deployment `dpl_F866gY3BgHgFLqiz8GCvJwqsJyiX` and validated locally through authenticated first upload; production activation remains blocked on an ordered migration deploy, private-bucket provisioning and administrator credentials. Ready to Hunt remains production-certified in the current live release.)
...
- Canonical species PRIMARY media is implemented locally. One service-role-only database relationship keyed by `species:*` drives the species library/profile and Hunt selector/result/map/Ready to Hunt consumers. The temporary administrator surface is `/admin/species-media`; successful Supabase Auth plus an explicit UUID allowlist produces an eight-hour signed HttpOnly cookie, and every upload is checked again server-side. Drag/drop takes identity only from the card, never the filename. The pipeline drops metadata by re-encoding, keeps one sanitized master plus avatar/card/profile WebP derivatives in a private bucket, records rights/audit fields, uses immutable asset paths, and atomically retires/replaces the old primary with optimistic concurrency. Missing media stays a meaningful placeholder.
- Production activation is **BLOCKED**, not claimed: migration `20260921113816_canonical_species_primary_media.sql` is not in the remote ledger, and `20260921093347_map_intelligence_foundation.sql` is also locally pending while newer migrations have already landed remotely. Do not run a blind `supabase db push`. Reconcile/apply the ordered pending migrations, provision the private bucket with `npm run provision:species-media`, and configure the documented admin variables first. Production still has 0/60 PRIMARY images.
...
- **The map is an exploration surface (2026-09-21, direction in `CLAUDE.md` §41A).** Code lives in `src/lib/hunt/exploration/` (domain, server) and `HuntMap.tsx`, `ZoneCard.tsx`, `ZoneCanvas.tsx`, `map/google-overlays.ts` (presentation).
  - **Labels.** Every drawn zone carries its authority's designation in its own terms (WMU 57, GHA 26), from the canonical layer (`officialTermShort` + designation), placed at the pole of inaccessibility of its largest part (`label-point.ts`) and decluttered per frame (`labels.ts`: selected first, then certified, then by on-screen room; full label, else bare designation, else nothing). Alberta publishes WMUs 718, 728 and 794 as 6, 3 and 3 records; `fetchLayerGeometry` now merges records by designation, so a multipart zone is one feature with one label (it was being drawn, keyed and would have been labelled per fragment).
...
- **Québec is certified and not served (Québec session, 2026-09-21).** Everything below is on `main`; only the serving steps remain, and each is the owner's decision (see Blocked).
  - Geometry: all 59 designations in PostGIS (9,509 polygons, 2,424,981 vertices), held NEEDS_VERIFICATION, with subdivided parts, boundary parts and stored drawings for fast lookup (`20260921034448`, `20260921035259`, `20260921085602`).
  - Parity against the ministry's WFS: 306 points, 0 disagreements, including Maniwaki → 10O (`fixtures/hunt/ca-qc-spatial-parity.json`).
...
- **Production measures boundary distance with the old function.** Hunt calls `resolve_management_zone` (v1), which measures distance on `ST_Boundary(geom)::geography`. That treats each long lon/lat edge as a great-circle arc, so the reported distance to a boundary is off by up to about 260 m in Ontario, about 16.5 m in Manitoba and 2 m or less in Alberta. It skews the near-boundary warning, not which zone a point is in: containment is decided on the full geometry. `zone_boundary_distance_meters` (`20260921085602`) segmentizes first and is correct, but `resolve_management_zone_v2` uses it only for zones with boundary parts, which today means Québec's. Swapping v2 in leaves Ontario, Manitoba and Alberta measured exactly as now — that identity is what was proven at 1,206 points. Fixing them is a separate one-line change in the no-derivatives path (`ST_Segmentize(ST_Boundary(zone.geometry), 0.01)`), to be made with a before/after diff over every sample point and a re-run of their regressions.
...
- The permanent media read model now supports one canonical PRIMARY image per production species without changing the content bundles. Population remains 0/60 in production until the pending schema/storage/admin configuration is activated and verified images are assigned.
...
### Production
- **Canonical species media application code is deployed, but the production feature is not activated as of 2026-09-21.** Commit `c731eab` is live in Vercel deployment `dpl_F866gY3BgHgFLqiz8GCvJwqsJyiX`; the canonical home, species library/profile, Hunt and administrator routes return 200. Local schema replay succeeded in the repository PostGIS certification database and in an isolated Supabase stack through migration `20260921113816`. Seven focused tests pass: metadata stripping and derivative bounds, invalid/oversized refusal, session allowlist/tamper/expiry, immutable four-object upload, atomic publication and failed-publication cleanup. A real Chromium session signed in through the local Supabase Auth service, showed 60 missing images, dropped a JPEG whose deliberately unrelated filename was ignored onto Ruffed grouse, received `IMAGE SET`, rendered the canonical `/api/species-media/{asset}/card` URL and reduced the missing count to 59. Docker Desktop then stopped before the automated replacement and all-consumer browser pass completed; those checks remain required after infrastructure recovery. No production schema, storage object or species relationship was changed.
