...
- **Known gap — a missing Hunt Brief has the same empty 404 body.** Brief IDs are dynamic, so `dynamicParams` is not available there. Tried and rejected: calling `notFound()` from `generateMetadata` (no change), and a segment-level `not-found.tsx` (no change, and it switched the page's robots from `noindex` to `index, follow` by inheriting the root layout's metadata — a 404 inviting indexing, reverted immediately). Crawlers still get the correct 404 status; a reader with JavaScript sees the page. The gap is readers without it, and on slow connections, which is the realistic case for a shared link opened outdoors. Needs the not-found decision made before the page renders.
...
- The production selector groups 60 compact options and searches common, scientific, French, alternate, category and hunter terminology while keeping the remaining research-only registry out of runtime publication. Only the regulatory coverage registry enables evaluation.
