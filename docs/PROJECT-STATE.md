...
- Hunt Brief live persistence requires a provisioned Supabase project with the two migrations applied. `HUNT_SHARE_RATE_LIMIT_SECRET` is configured, but `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are absent. Missing storage fails closed with `503`; no share URL is claimed until the snapshot is durably stored.
