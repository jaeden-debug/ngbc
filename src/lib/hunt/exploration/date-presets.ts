import { isoParts, MONTH_NAMES, todayIso, WEEKDAY_NAMES, weekdayOf } from "../date.ts";

/**
 * The quick date choices, and how the chosen day is named.
 *
 * One preset only: Today (owner decision, 2026-09-22). Anything else is chosen
 * on the calendar. "This weekend" would be the worst of them — a Hunt is
 * checked for one day, the weekend is two, and Sunday rules differ from
 * Saturday's in several jurisdictions (Alberta closes big game on Sundays in
 * many units), so silently picking Saturday would answer a question the hunter
 * did not ask.
 *
...
export type DatePreset = "today";

export function presetDate(_preset: DatePreset, now: Date = new Date()): string {
  return todayIso(now);
}
...
export function presetOf(iso: string, now: Date = new Date()): DatePreset | null {
  return iso === presetDate("today", now) ? "today" : null;
}
...
/** The chip's words: "Today", or the short day. */
export function dateChipLabel(iso: string, now: Date = new Date()): string {
  return presetOf(iso, now) === "today" ? "Today" : shortDayLabel(iso, now);
}
