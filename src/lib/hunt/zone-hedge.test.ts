...
import { reconcileZoneAnswers, resolveZone, SUPABASE_ZONE_TIMEOUT_MS, ZONE_HEDGE_DELAY_MS } from "./zone.ts";
...
test("two answers in hand that disagree are a question for a person, not a pick", () => {
  ...
  assert.equal(reconcileZoneAnswers(a, { ...a }), a, "agreement uses the first");
  for (const [left, right] of [[a, b], [a, none]] as const) {
    const result = reconcileZoneAnswers(left, right);
